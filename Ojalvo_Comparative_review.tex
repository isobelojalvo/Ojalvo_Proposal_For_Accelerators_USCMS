...
machine learning based algorithms. %fix me
...
Our group is already 

For track finding, we consider only tracks and hits in the barrel region of the detector. 
...
filter that is tuned to be efficient for tracks resulting from high transverse momentum
particles. Figure %%ref figure
shows 
...
to a highly parallel computing architecture making use of CPUs which have an FPGA co-processor. We have access to an Intel Xenon CPU + FPGA co-processor at computing 
facilities at CERN and two similar devices are planned to be acquired at the 
Princeton Research Computing center. We are excited to share progress regularly 
in DataX forums and to collaborate with members of both the physics and computer science communities for advancement. For implementation, we will modify the existing cluster and track seed finding steps by developing a shallow NN to identify and disentangle shared clusters and then link clusters in neighboring layers to create primitive tracks. Monte Carlo truth information, which models the helical path of the charged particle through the detector, will be used for the training. We plan to test the NN performance on simulated events with 10 tracks, 100 tracks and 500 tracks. The completion of this project will result in the mapping out of resource usage and latency for various ranges of input data and network architectures.
Upon successful completion of this project, our group will be the first to complete a NN based approach to track hit clustering and track seed reconstruction using an FPGA co-processor architecture, which will help maximize the HL-LHC physics reach. We will use these first preliminary studies to submit proposals to the DOE and to the NSF which will fully utilize FPGA co-processors to accelerate online object reconstruction at the CMS experiment. Specifically we will submit proposals to both continue the algorithmic research into using FPGAs (DOE Office of Science, SciDAC and ASCR, NSF PHY and OAC), and also for testbed compute resources (e.g. NSF CC* and/or MRI Track 1) as well as an eventual potential hardware upgrade (NSF MRI Track 2) for the CMS experiment.
